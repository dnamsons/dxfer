...
use svgx::{
    document::Document,
    nodes::{values::*, Circle, Ellipse, Line, Path, Polyline, Text},
};
...
fn entity_to_node(
    ...
    let layer = drawing.layers().find(|layer| layer.name == common.layer);

    let should_draw = layer
        .and_then(|layer| Some(layer.is_layer_plotted && layer.is_layer_on))
        .unwrap_or(false);
    ...
    let layer_name = layer.unwrap().name.clone();
    document.add_layer(layer_name.clone());
    ...
    match &entity.specific {
        EntityType::LwPolyline(polyline) => {
            ...
            document.add(node, Some(layer_name));
        }
